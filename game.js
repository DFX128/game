...
const page = new VK(); // создание страничного бота
 page.setOptions({token: `dfac32621fdc3acfda035aeece84a93b9f14f539965e985d4380de35150e784adf89c42dc0fedb9eb943e`}); // токен вашей страницы
...
const widgettoken = "1adab84b28dafb348fba386aa04f73076f86eeaf8ae294fe998a769a0c66946f54ecb536ae8c9954c06a8" //токен группы с доступом к виджетам
...
vk.setOptions({ 
	token: "f5a50ea2f702bb6cebaad0958450a11839ead52023dac53039ebf234671f541d74ef20b1625d87507e338",  // токен группы (бота)
	apiMode: "parallel", // привед
	pollingGroupId: 191263043 // ид группы (бота)
});

const thisbotname = `gamebot1357` // имя вашего бота
const thisbotid = 191263043 // ид группы (бота)
const thisbotbeseda = `https://vk.me/join/AJQ1d4NlXhYEif3Ka3SyvjO9` // беседа вашего бота (ссылка, пример: https://vk.me/join/AJQ1d3ZZwxVAf3/E4sHsuyrI)
const thistelegabot = `TELEGRAMbot` // ссылка на вашего телеграмм бота (пример: joraTELEGRAMbot)
// Настройки бота //



// MCPETRADE ——— Автодонат (обработчик и сайт не нужен, всё находится в этом коде, достаточно просто указать значения)
const vipdesc = `✅1.Увеличен лимит передачи валюты другим игоокам (х5)
✅2.Lite отметка в профиле
✅3.Увеличен лимит ферм до 2000
✅4.Возможность ставить ник на 5 символов длиннее»`; // описание випки / ссылка на товар, пример: vk.com/market-187706737?w=product-187706737_3495265
const premdesc = `✅1.Увеличен лимит передачи другим игрокам х10
✅2.Vip отметка в профиле
...
у премиум только одна: им свой_ид название_имущества новое_название - сменяет имущество премиум игроку.»`; // описание премки / ссылка на товар, пример: vk.com/market-187706737?w=product-187706737_3495265
...
const mcpetrade = {
"shop": 130254, // ид магазина
"server": 15122, // ид сервера
"premium": 58726, // Имя данного товара должно быть rank3 на mcpetrade//product , ид товара "Premium" в вашем магазине.
"vip": 58734, // Имя данного товара должно быть rank2 на mcpetrade//product , ид товара "V.i.P" в вашем магазине.
"studio": 58735, //Имя данного товара должно быть biznes20 на mcpetrade// product , ид товара "Бизнес Киностудия" в вашем магазине.
"express": 58736, // Имя данного товара должно быть biznes21 на mcpetrade//product , ид товара "Бизнес Межпланетный Экспресс" в вашем магазине.
"jorik": 58737 // Имя данного товара должно быть pet8 на mcpetrade//product , ид товара "Лучший питомец Жорик" в вашем магазине.
}
...
if(context.payload.message.attachments[0]) {
	...
	if(context.payload.message.attachments[0].market.title == `Питомец «Марабу»`) {
	...
	if(context.payload.message.attachments[0].market.title == `Lite статус`) {
...
🔸Цена: 39(без скидки 59)₽. Для покупки введите: «донат 1» 🎁
...
🔸Цена: 159(без скидки 289)₽. Для покупки введите: «донат 2» 🎁

3.
🦅 Питомец «Марабу», самый лучший питомец.
🔹При максимальном уровне приносит до 500,000,000,000,000$
🔹Марабу невозможно потерять в походе
🔹Марабу устаёт всего на 15 минут вместо 60-ти.
🔸Продать Марабу можно за 40,000,000,000,000$
🔸Цена: 49(без скидки 99)₽. Для покупки введите: «донат 3» 🦅

4.🔮 Статус «Vip», самый лучший донат статус.
🔹Подробное описание здесь: «${premdesc}»
🔸Цена: 149(без скидки 299)₽. Для покупки введите: «донат 4» 🛍️

5.🔥 Статус «Lite», самый дешёвый донат статус.
🔹Подробное описание здесь: «${vipdesc}»
🔸Цена: 29(без скидки 59)₽. Для покупки введите: «донат 5» 🛍️
...
if(choose == 1) {
setTimeout(() => context.send({chat_id: context.chatId, message: `>> А вы знали, что добавляя [public191263043|gamebot1357] в беседу, Вы получаете крутые награды в виде доната, денег и т.п? 🤑
...
if(choose == 2) {
setTimeout(() => context.send({chat_id: context.chatId, message: `🎁 В нашей [public191263043|ГРУППЕ] конкурсы и раздачи каждый день! Подпишись и получи шанс выиграть крутые призы `,
      keyboard: Keyboard.keyboard([
      [ 
            Keyboard.textButton({ 
            label: 'https://vk.com/gamebot1357', 
            color: Keyboard.POSITIVE_COLOR,
            payload: {
            	"command": "help"
            }
...
setInterval(function (){
let resphoto = `photo-191263043_457239042`
let hour = new Date().getHours()
let minute = new Date().getMinutes()
let second = new Date().getSeconds()
...
if(time == `12:00`) {
	if(base.context.lastgiveawayid !== 0) {
		let reposts = 0
		page.api.wall.getReposts({owner_id: -191263043, post_id: base.context.lastgiveawayid, count: 1000}).then((res) => {
...
if(givetype == 1) {
	page.api.wall.post({owner_id: -191263043, attachment: resphoto, message: `📢 Началась новая раздача!
...
for(let z in chats.ids){
	if(chats.ids[z].active !== 3) {
		vk.api.messages.send({chat_id: chats.ids[z].id, attachment: `wall-191263043_${res.post_id}`, message: `[УВЕДОМЛЕНИЕ]\n📢 Началась новая раздача!\n`, })
}		
}
})
} // раздача #1
...
if(givetype == 4) {
	...
	page.api.wall.post({owner_id: -191263043, message: `⏳ Новый промокод!
...
updates.hear(/промо Марабу|промокод Марабу|промо Марабу|промокод Марабу$/i,(context) => {
let platform = false
if(context.isChat) platform = true
let user = base.bs[base.id[context.senderId].id]
let nick = ``
...
context.send(`${nick}, Вы активировали Промокод <<Марабу>>.
...
vk.updates.on('new_wall_post', async (context) => {
   ...
   if(context.payload.text.includes(`изменения`)) return
        if(context.payload.text.includes('biznes21')){
            ...
            if(!base.bs[tels]) return page.api.call("wall.delete", {owner_id: -191263043, post_id: context.payload.id})
        ...
        if(context.payload.text.includes('rank3')){
            ...
            if(!base.bs[tels]) return page.api.call("wall.delete", {owner_id: -191263043, post_id: context.payload.id})
            vk.api.messages.send({chat_id: adminchat, message: `@id${base.bs[tels].id} купил статус «🎗 Vip», за 299₽`})
            base.bs[tels].rank = Number(3)
            vk.api.messages.send({user_id: base.bs[tels].id, message: `🛍️ Благодарим вас за покупку самого лучшего донат статуса в боте: «🎗 Vip», подробнее: «${premdesc}» 💰\n🔸 Напишите: <</Vip», Для просмотра команд.`})
            page.api.call("wall.delete", {
				owner_id: -191263043,
			    post_id: context.payload.id
				})
    ...
    if(context.payload.text.includes('rank2')){
            ...
            if(!base.bs[tels]) return page.api.call("wall.delete", {owner_id: -191263043, post_id: context.payload.id})
            vk.api.messages.send({chat_id: adminchat, message: `@id${base.bs[tels].id} купил статус «🔥 Lite», за 59₽`})
            base.bs[tels].rank = Number(3)
            vk.api.messages.send({user_id: base.bs[tels].id, message: `🛍️ Благодарим вас за покупку самого дешёвого донат статуса в боте: «🔥 Lite», подробнее: «${vipdesc}» 💰🔸 Напишите: <</lite, Для просмотра команд.`})
            page.api.call("wall.delete", {
				owner_id: -191263043,
			    post_id: context.payload.id
				})
    ...
    if(context.payload.text.includes('pet8')){
            ...
            if(!base.bs[tels]) return page.api.call("wall.delete", {owner_id: -191263043, post_id: context.payload.id})
            vk.api.messages.send({chat_id: adminchat, message: `@id${base.bs[tels].id} купил питомца 🦅 «Марабу», за 99₽`})
            base.bs[tels].petid = Number(8)
            base.bs[tels].petname = `Марабу`
            base.bs[tels].peticon = `🦅`
            base.bs[tels].petlvl = Number(1)
            base.bs[tels].pettimer = Number(0)
            vk.api.messages.send({user_id: base.bs[tels].id, message: `🛍️ Благодарим вас за покупку самого лучше питомца в боте: «🦅 Марабу», подробнее: «питомец» 💰`})
            page.api.call("wall.delete", {
				owner_id: -191263043,
			    post_id: context.payload.id
				})
...
if(user.rank > 1) {
let status = ``
if(user.rank == 2) status = `🔥 Lite`
if(user.rank == 3) status = `🎗 Vip`
...
✏  Основные команды:
 ...
 🍹 Зелья
 💲 Баланс
 💰 Банк⠀» 💳 Банк помощь - все команды банка
 👑 Рейтинг - ваш рейтинг
 🏆 Топ
✒ Ник [ник/вкл/выкл]
 🛍 Магазин
...
3.🦅 Питомец «Марабу», самый лучший питомец.
🔹При максимальном уровне приносит до 500,000,000,000,000$
🔹Марабу невозможно потерять в походе
🔹Марабу устаёт всего на 15 минут вместо 60-ти.
🔸Продать Марабу можно за 40,000,000,000,000$
...
4.🎗 Статус «Vip», самый лучший донат статус.
🔹Подробное описание здесь: «${premdesc}»
🔸Цена: 149(без скидки 299)₽. Для покупки введите: «донат 4» 🛍

5.🔥 Статус «Lite», самый дешёвый донат статус.
🔹Подробное описание здесь: «${vipdesc}»
🔸Цена: 29(без скидки 59)₽. Для покупки введите: «донат 5» 🛍
...
let myclan = clan[base.bs[base.id[context.senderId].id].clan]
	if(user.rank < 2) return
		return context.send(`${nick}, Команды Lite игрока в разработке...`)
})
...
bot.on('message', async (ctx, next) => {
	if(!tlgrm[ctx.from.id]) {
	 ...
	 ctx.reply(`${ctx.from.first_name}, Приветствую тебя! При помощи меня ты сможешь привязать свой аккаунт в vk.com/gamebot1357 к своему телеграмм аккаунту и получить плюшки.
😙 Для получения инструкций по привязке напиши vk.com/gamebot1357 смс с текстом: <</tlgrm>>`)
	}
    await next();
})
...
menu.urlButton('JOPA BOT', `Игровой бот`)
...
if(context.isChat) platform = true
let user = base.bs[base.id[context.senderId].id]
let nick = ``// Created by vk.com/keyfqs 
if(user.nicknotify == false) {
	nick = `${base.bs[base.id[context.senderId].id].nick}`
}
if(user.nicknotify == true) {
...
async function run() {
    ...
    console.log(chalk.red.bold.underline(`Created by vk.com/Alexey1357`))
}
